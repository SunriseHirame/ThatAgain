...
namespace Game
{
    public class AnimationController : MonoBehaviour
    {
        ...
        [SerializeField] private KinematicController attachedController;
...
*/
            ...
            attachedAnimator.SetFloat (Anim_MoveSpeed, math.abs (xVelocity) * speedMult);
            attachedAnimator.SetBool (Anim_InAir, !attachedController.OnGround);

            if (!inAir && !attachedController.OnGround)
            {
                inAir = true;
                attachedAnimator.SetTrigger (Anim_Jump);
            }
            else if (inAir && attachedController.OnGround)
            {
                inAir = false;
            }
        }
